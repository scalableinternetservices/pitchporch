...
usage="Usage: deploy-ecs.sh [pitchporch-app-web|pitchporch-app-background] [sha]"
...
new_image="101624687637.dkr.ecr.us-west-2.amazonaws.com/pitchporch:$2"
...
aws ecs update-service --cluster pitchporch \
  --no-cli-pager \
  --region "$ecs_region" \
