...
resource "aws_ecs_cluster" "pitchporch" {
  name = "pitchporch"
}

resource "aws_ecr_repository" "pitchporch" {
  name                 = "pitchporch"
...
module "webserver" {
  ...
  ecr_repository = aws_ecr_repository.pitchporch.repository_url
  ecs_cluster    = aws_ecs_cluster.pitchporch.id
